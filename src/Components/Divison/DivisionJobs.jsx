...
const DivisionJobs = () => {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (selectedDivisions.length > 0) list = list.filter((job) => selectedDivisions.includes(job.division));
    else list = list.filter((job) => job.division === decodedDivision);

    if (selectedCategories.length > 0) list = list.filter((job) => selectedCategories.includes(job.category));
    ...
    return list;
  }, [initialDivisionJobs, selectedCategories, selectedDivisions, sortBy, decodedDivision]);
  ...
  return (
    <div className="max-w-6xl mx-auto mt-4 px-6 md:px-6 lg:px-0">

      {/* Top header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-green-600">{decodedDivision}</h1>
          <p className="text-sm text-gray-500">
            Showing {filteredAndSorted.length} job{filteredAndSorted.length !== 1 ? "s" : ""} {selectedCategories.length > 0 || selectedDivisions.length > 0 ? "(filtered)" : ""}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {selectedCategories.map(c => <div key={c} className="flex items-center gap-2 bg-blue-50 border text-blue-800 px-3 py-1 rounded-full text-sm">
          <span>{c}</span><button onClick={()=>removeTag("category",c)}><AiOutlineClose /></button>
        </div>)}
        {selectedDivisions.map(d => <div key={d} className="flex items-center gap-2 bg-green-50 border text-green-800 px-3 py-1 rounded-full text-sm">
          <span>{d}</span><button onClick={()=>removeTag("division",d)}><AiOutlineClose /></button>
        </div>)}
        {(selectedCategories.length>0 || selectedDivisions.length>0) &&
          <button onClick={handleClearFilters} className="ml-2 text-sm text-gray-600 underline">Clear All</button>
        }
      </div>
          ...
          <div className="bg-white border rounded-xl shadow-sm p-5">
            ...
            <div className="mb-4">
              <button onClick={()=>setIsCatOpen(s=>!s)} className="w-full flex items-center justify-between p-2 rounded-md hover:bg-gray-50">
                ...
                <span className="text-gray-500">{isCatOpen ? "−" : "+"}</span>
              </button>
              {isCatOpen && <div className="mt-3 max-h-56 overflow-auto pr-2">
            ...
            <div className="mb-2">
              <button onClick={()=>setIsDivOpen(s=>!s)} className="w-full flex items-center justify-between p-2 rounded-md hover:bg-gray-50">
                ...
                <span className="text-gray-500">{isDivOpen ? "−" : "+"}</span>
              </button>
              {isDivOpen && <div className="mt-3 max-h-56 overflow-auto pr-2">
                {divisions.map((div,idx)=>(
                  <label key={idx} className="flex items-center gap-2 py-1">
                    <input type="checkbox" checked={selectedDivisions.includes(div)} onChange={()=>toggleDivision(div)} />
            ...
            <div className="mt-4">
              <button onClick={handleClearFilters} className="w-full py-2 rounded-md bg-gray-100 hover:bg-gray-200">Clear Filters</button>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0">
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 mt-2">Try removing filters or changing sort.</p>
            </div>
          ) : (
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between mt-6 gap-2">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  {Array.from({length: totalPages}).map((_,idx)=>{
                    const pageNum = idx+1;
                    return (
                      <button key={pageNum} onClick={()=>setCurrentPage(pageNum)} className={`px-3 py-1 rounded-md border ${pageNum===currentPage?"bg-blue-600 text-white":"hover:bg-gray-100"}`}>{pageNum}</button>
                    );
                  })}
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative w-full max-h-[90vh] bg-white rounded-t-xl shadow-xl p-4 overflow-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={()=>setMobileFiltersOpen(false)} className="p-1 rounded-md"><AiOutlineClose /></button>
            </div>

            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-medium">Category</h4>
                <button className="text-sm text-gray-600" onClick={()=>setSelectedCategories([])}>Clear</button>
              </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-sm text-gray-600" onClick={()=>setSelectedDivisions([])}>Clear</button>
              </div>
              <div className="max-h-56 overflow-auto pr-2">
                {divisions.map((div,idx)=>(
                  <label key={idx} className="flex items-center gap-2 py-1">
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={()=>{handleClearFilters(); setMobileFiltersOpen(false)}} className="flex-1 py-2 rounded-md bg-gray-100">Clear</button>
              <button onClick={()=>setMobileFiltersOpen(false)} className="flex-1 py-2 rounded-md bg-blue-600 text-white">Apply</button>
            </div>
          </div>
        </div>
      )}

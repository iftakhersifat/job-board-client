    ...
    <div className="max-w-6xl mx-auto mt-24 px-6 lg:px-0">
      ...
      <div className="text-center px-6">
        <h1 className="text-4xl md:text-5xl font-extrabold mt-6 tracking-tight">
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-500 drop-shadow-sm">
            Hot Jobs
          </span>
        </h1>

        <div className="w-28 h-1 bg-gradient-to-r from-indigo-500 via-blue-600 to-cyan-500 mx-auto mt-4 rounded-full shadow-sm"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10">
        {jobs
          ...
          .map((job) => (
            <div
              key={job._id}
              className="group relative flex flex-col justify-between bg-white/60 backdrop-blur-md rounded-3xl shadow-lg hover:shadow-2xl h-[400px] transition-all duration-500 p-6 border border-transparent hover:border-indigo-300"
            >
              {/* Featured Badge - Blue/Indigo theme */}
              {job.featured && (
                <span className="absolute top-4 right-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                  Featured
                </span>
              )}
              ...
              <div className="flex-1 space-y-3">
                <h2 className="font-bold text-xl text-gray-800 group-hover:text-indigo-600 transition-colors leading-snug">
                  {job.title}{" "}
                ...
                {/* Skills - Now using Indigo tags */}
                <div className="flex flex-wrap gap-2 mt-2 max-h-24 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-200 scrollbar-track-indigo-50">
